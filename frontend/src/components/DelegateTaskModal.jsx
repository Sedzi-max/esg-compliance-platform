...
function DelegateTaskModal({ isOpen, onClose, defaultTaskName, defaultFacility, frameworkCode, frameworkName, onSuccess }) {
  ...
  // FIX: the modal instance is reused across different gap rows. Without this,
  // reopening it for a different clause kept showing the previous clause's
  // task_name/facility_name until the whole component remounted.
  useEffect(() => {
    if (isOpen) {
      setFormData(prev => ({
        ...prev,
  ...
  }, [isOpen, defaultTaskName, defaultFacility]);
  ...
  const handleDispatchEmail = async (e) => {
    ...
    try {
      ...
      await axios.post('/api/notify/delegate', {
        ...formData,
      ...
      if (onSuccess) onSuccess(); // let the parent refresh the gap table
